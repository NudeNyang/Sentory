...
public sealed class SlackContextValidatorTests
{
    [Fact]
    public void AcceptsForegroundSlackRenderer()
    {
        var native = new FakeNative();
        var validator = new SlackContextValidator(native, native);
    ...
    [Fact]
    public void RejectsWindowWithoutChromiumRenderer()
    {
    ...
    private sealed class FakeNative : INativeWindowApi, IDiscordWindowApi
    {
        ...
        public string ProcessName { get; set; } = "Slack";

        public nint FindDescendant(nint root, string className) =>
            className == SlackContextValidator.RendererClassName
                ? Renderer
                : nint.Zero;
