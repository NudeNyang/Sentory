...
public enum SlackNativeDropRegistrationResult
{
    Registered,
    Paused,
    TargetInvalid,
...
public sealed class SlackCaptureRuntime : ICaptureRuntime
{
    ...
    private readonly ISlackAccessibilityClient _accessibility;
        ...
        Action<string, string>? diagnostic = null)
    {
        ...
        _native = native;
        _validator = new SlackContextValidator(native, native);
        _hook = new LowLevelPasteHook(native, acceptInjectedInput);
        _clipboardReader = new StaClipboardReader(native);
        _accessibility = new SlackAccessibilityClient(diagnostic);
    ...
    private void OnSendDetected(object? sender, PasteTrigger trigger)
    {
        ...
        {
            _diagnostic?.Invoke(
                "slack-send-key-observed",
                $"candidates={observed}");
        }
    }

    public async Task<SlackNativeDropRegistrationResult>
            ...
            IReadOnlyList<string> paths)
    {
        if (_paused)
        {
            return SlackNativeDropRegistrationResult.Paused;
        }
        ...
        if (!_validator.TryValidate(
                ...
                out var context))
        {
            return SlackNativeDropRegistrationResult.TargetInvalid;
        }
        ...
        {
            return SlackNativeDropRegistrationResult.UnsupportedFiles;
        }
        ...
        try
        {
            ...
            if (baseline is null)
            {
                return SlackNativeDropRegistrationResult
                    .ConversationUnavailable;
            }
            ...
            if (images.Count == 0)
            {
                return SlackNativeDropRegistrationResult.ImageReadFailed;
            }
            ...
            var registered = StartCandidate(
                ...
                nativeDrop: true);
            _diagnostic?.Invoke(
                "slack-drop-candidate",
                $"registered={registered} files={imagePaths.Length} images={images.Count}");
            return registered
                ? SlackNativeDropRegistrationResult.Registered
                : SlackNativeDropRegistrationResult.Duplicate;
        }
        ...
        catch (Exception exception)
        {
            ...
            return SlackNativeDropRegistrationResult.Failed;
        }
    }
    ...
    private async Task RunCandidateAsync(CandidateRegistration registration)
    {
        try
        {
            var response = await _accessibility.WaitForConfirmationAsync(
                new SlackConfirmationRequest(
                    registration.Context,
                    registration.Baseline,
                    registration.Urls,
                    registration.Images
                        .Select(image => image.OriginalFileName)
            ...
            var result = await _coordinator.CaptureBatchAsync(
                ...
                SourceApp.Slack,
                registration.NativeDrop
                    ? CaptureMethod.SlackConfirmedDrop
                    : registration.Images.Count > 0
                        ? CaptureMethod.SlackConfirmedImage
                        : CaptureMethod.SlackConfirmedSend,
    ...
    private void ReportIssue(Exception exception)
    {
        ...
        IssueDetected?.Invoke(
            ...
            new CaptureRuntimeIssue(
                "slack-capture-item-failed",
                "Slack 입력 일부를 처리하지 못했지만 감지는 계속됩니다.",
                now));
    }
        ...
        CancellationTokenSource cancellation)
    {
        ...
        public ValidatedSlackContext Context { get; } = context;
        public SlackAccessibilitySnapshot Baseline { get; } = baseline;
