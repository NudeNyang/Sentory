...
| --- | --- | --- |
| Discord | 붙여넣거나 `+`로 고른 사진·링크를 실제로 전송한 뒤 | 붙여넣기만 한 경우, `Shift+Enter`, 첨부 취소 |
| 카카오톡 | 개별 채팅창에 링크·사진을 붙여넣거나 탐색기 드롭·`+` 파일 선택으로 사진을 전달했을 때 | 직접 입력한 주소, 검색창 입력, 채팅창 밖의 입력 |
...
2. 카카오톡 개별 채팅창에 링크나 사진을 붙여넣거나 `+`에서 사진 파일을
   선택합니다. Discord에서는 붙여넣거나 `+`로 고른 사진을 실제로 전송해야
   저장됩니다.
