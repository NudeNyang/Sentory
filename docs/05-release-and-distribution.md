...
| `Sentory-2.0.7-source.zip` | 해당 바이너리에 대응하는 전체 소스 |
...
10. `v2.0.7` 태그가 배포 파일을 만든 커밋을 가리키는지 확인합니다.
...
끝나기 전의 Release와 2.0.7 이하 파일은 미서명 상태로 유지한다.

## 버전과 업데이트

Sentory는 의미적 버전 형식을 사용한다. 2.0.7 Tauri판은 GitHub Releases에서 같은
