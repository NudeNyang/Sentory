...
10. `v2.0.1` 태그가 배포 파일을 만든 커밋을 가리키는지 확인합니다.
...
Sentory는 의미적 버전 형식을 사용한다. 2.0.1 Tauri판은 설정의 `지금 확인`을
