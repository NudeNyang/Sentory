...
2. 설정의 앱 정보에 `자동 업데이트`가 없는지 확인한다.
   앱 안에는 제품 버전 `2.0.10`을 표시하고 Windows의 설치된 패키지 정보에는
   MSIX 버전 `2.0.10.0`을 표시한다.
...
```powershell
.\scripts\Publish-MsixStoreBundle.ps1 -PackageVersion 2.0.10.0
```

기본 출력은 `artifacts/store`이다.

- `Sentory-2.0.10-store.msixbundle`: Partner Center에 올릴 파일
- `Sentory-2.0.10-store.msixbundle.sha256`: 번들 확인값
- `Sentory-2.0.10.0-store-x64.msix`: x64 개별 패키지
- `Sentory-2.0.10.0-store-arm64.msix`: ARM64 개별 패키지
...
패키징 스크립트는 MakeAppx의 `/bv` 값에도 `2.0.10.0`을 넘긴다. 따라서 개별 x64·
...
Packages 화면에는 `Sentory-2.0.10-store.msixbundle` 하나만 올린다. 개별 MSIX,
...
2.0.10.0 매니페스트에는 `unvirtualizedResources`가 없으므로 이 기능에 대한 제한
