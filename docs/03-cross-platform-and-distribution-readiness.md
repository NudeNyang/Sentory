...
2.0.10 정식판은 Tauri 호스트와 self-contained C# 엔진을 묶은 x64·ARM64 설치형과
...
명령을 백업하고 Sentory 프로세스가 먼저 시작된 뒤 접근성 옵션으로 Discord를 여는 명령으로
관리한다. `NudeNyang Translator`도 Windows 자동 실행에 등록돼 있으면 해당
프로세스까지 기다린다. 두 기능 중 하나를 끄면 기존 값을 복원하며, 30초 안에
Sentory가 시작되지 않아도 관리 명령 자체가 원래 값을 복원하고 Discord를 실행한다.
설치형 제거 프로그램도 Sentory 시작 항목을 삭제하기 전에 Discord 시작 명령
복원을 실행한다. 이 레지스트리 연동은 Windows 플랫폼 계층에만 둔다. MSIX판은
Sentory 자동 실행에는 `StartupTask`를 사용하되, Discord 시작 값과 백업 키는
패키지 밖에서 공유하도록 매니페스트의 레지스트리 가상화 예외를 사용한다.
