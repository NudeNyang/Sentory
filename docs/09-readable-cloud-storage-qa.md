...
6. 삭제 전에 이미 실행한 구버전에서 지운 항목은 삭제 기록이 없으므로 자동
   ...
   삭제해 검증한다.
7. 클라우드의 `Sentory` 폴더를 통째로 삭제한 직후 메인 컴퓨터에서 기존 사진
   하나를 삭제하고 새 사진 하나를 추가한다. 자동 재구축과 양쪽의 최근 동기화
   완료 뒤에는 삭제한 사진이 어느 쪽에도 되살아나지 않고 새 사진만 VM에
   나타나는지 확인한다.

## 8. 즐겨찾기·사용 횟수·자동 즐겨찾기 확인

1. 양쪽 Sentory를 같은 동기화 폴더에 연결하고 같은 검수용 사진이나 링크가
   보이는 상태를 만든다.
2. 메인 컴퓨터에서 해당 항목을 두 번 복사하고 VM에서 한 번 복사한다.
3. 양쪽의 `최근 동기화 완료`를 확인한 뒤 두 보관함 모두 복사 횟수가 3회인지
   확인한다.
4. 한쪽에서 즐겨찾기에 추가하고 반대쪽에 반영되는지 확인한 다음, 반대쪽에서
   해제해 처음 컴퓨터에도 해제가 돌아오는지 확인한다.
5. 자동 즐겨찾기를 2회로 켠 뒤 다른 컴퓨터의 설정에도 같은 값이 반영되는지
   확인한다. 서로 다른 두 컴퓨터에서 한 번씩 복사한 새 항목이 양쪽에서 자동
   즐겨찾기가 되는지도 확인한다.
6. 외부 전송 검수는 같은 날 연속 전송을 여러 번 해도 한 번으로 계산되는지,
   6시간 넘게 떨어진 두 번째 전송이 별도 사용으로 계산되는지 확인한다.
7. 마지막으로 한쪽에서 해당 항목을 삭제하고, 늦게 도착하는 복사·즐겨찾기
   정보 때문에 어느 쪽에서도 다시 생기지 않는지 확인한다.
