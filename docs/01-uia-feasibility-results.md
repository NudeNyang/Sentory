...
2026-07-27에 네이티브 파일 선택창의 `열기` 동작과 선택된 로컬 이미지 경로를
관찰하는 경로를 추가했다. 선택창의 소유 창이 검증된 Discord 채팅창인지 먼저
확인하고, 실제 이미지 파일만 메모리에 읽는다. 이후 키보드 Enter나 마우스 전송
버튼 자체를 신뢰하지 않고 기준점 뒤에 새 본인 사진 메시지가 나타난 경우에만
`DiscordConfirmedImage`, `Confirmed`로 저장한다. 파일 선택창을 취소하거나
사진 초안을 제거하고 보내지 않으면 2초 유예 뒤 후보를 취소해 저장하지 않는다.

첫 개발자판의 실제 검수에서는 파일 선택창과 Discord 소유 관계까지는
확인됐지만 `EVENT_OBJECT_INVOKED`가 발생하지 않아 `decision=Unknown`으로
끝났고, 탐색기가 확장자를 숨긴 파일명을 노출해 선택 경로도 0개였다. 이후
`열기`·`취소` 버튼의 실제 마우스 클릭과 Enter·Esc를 저수준 입력 훅으로
구분하고, 현재 폴더에서 확장자를 붙인 후보가 정확히 하나일 때만 실제 경로로
복원하도록 수정했다. 초안이 한 번도 나타나지 않는 후보는 5초 뒤 폐기한다.
자동 검증은 완료했으며 실제 Discord 재검수는 남아 있다.

같은 날 두 번째 실검수에서 단일 사진을 한 번 클릭하면 선택창이 즉시
확정되는 동작과, 여러 사진을 선택한 뒤 `열기`를 누르는 동작 모두 여전히
`decision=Unknown selected=0`으로 끝났다. 120ms UIA 폴링과 버튼 호출
이벤트를 주 경로로 삼는 구조 자체가 짧은 선택 상태를 놓친 것이 원인이었다.
이후 Windows의 `EVENT_OBJECT_SELECTION`, `SELECTIONADD`,
`SELECTIONREMOVE`, `SELECTIONWITHIN`, `VALUECHANGE`에서 MSAA 항목을 즉시
읽고 단일·다중 파일명을 누적하도록 변경했다. 선택 직후 1초 안에 창이
자동으로 닫히면 실제 존재하는 지원 이미지 경로가 확인된 경우에만 확정하며,
명시적인 `취소`·Esc는 항상 우선한다. 파일명 원문은 진단 로그에 남기지 않는다.
