...
막지 않고 격리 워커에서 비동기로 수행되며 현재 환경에서는 보관함 반영까지
약 4~6초가 걸렸다.

진단 스크립트와 현재 격리 워커는 후보마다 접근성 트리를 다시 찾아 수 초가
걸린다. 장기 실행 격리 워커의 안전한 대상 캐시나 WinEventHook 상관관계가
검증되면 지연을 줄일 수 있다. 깊이 우선 탐색을 단순 너비 우선 탐색으로
교체하는 방식은 다른 목록을 먼저 선택해 판정이 누락됐으므로 사용하지 않는다.
메시지 전송 자체는 기다리지 않고 확인과 저장을 비동기로 수행한다.
